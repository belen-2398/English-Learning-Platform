...
@section('title', 'Explanation Edit')
@section('content')
<div>
    @if ($errors->any())
    ...
    <div class="m-6 p-4 flex justify-between">
        <h2 class="head-title">Edit the explanation</h2>
    </div>
    <form action="{{ route('explanations.update', $explanation->id ) }}" method="POST"
        ...
        <div class="flex justify-between gap-12 mt-4">
            ...
            <div class="flex items-center mb-6">
                <textarea name="explanation" id="explanation" cols="100" rows="10"
                    class="bg-gray-200 whitespace-pre-wrap appearance-none border-2 border-gray-200 rounded py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500">{{ $explanation->explanation }}</textarea>
            </div>
        </div>
