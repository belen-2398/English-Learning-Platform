<template>
    ...
    <template v-if="$page.props.auth.user">
        <template v-if="$page.props.completed">
            <div>
                <div class="mt-16 text-center flex justify-center gap-10 items-center">
                    <div class="border-4 border-dashed rounded-full p-6">
                        <h3 class="underline underline-offset-8">Your points</h3>
                        <p class="my-4 text-2xl">{{ totalUserPts }}</p>
                    </div>
                    <div>
                        <p class="my-4 text-lg">{{ userPtCategory }}</p>

                        <div class="leading-tight">
                            <p>Points until next category: {{ nextCatPts }}</p>
                            <div class="w-3/4 mx-auto bg-[var(--color-light)] rounded-full dark:bg-gray-700">
                                <div class="bg-[var(--color-medium1)] text-xs font-medium text-blue-100 text-center p-1.5 leading-none rounded-full"
                ...
                <div
                    class="mt-16 flex flex-col border-2 border-dashed border-[var(--color-medium1)] p-4 mx-8 justify-between">
                                ...
                                <div class="inline-flex items-top">
                                    ...
                                    <div>
                                        ...
                                        <div v-for="topic in topicsByLesson" :key="topic.id">
                                            ...
                                            <p class="text-xs mt-1 whitespace-nowrap text-[var(--color-darker)]">
                                                {{ topic.points }} pts.
                                            </p>
                                            </Link>
                                        </div>
