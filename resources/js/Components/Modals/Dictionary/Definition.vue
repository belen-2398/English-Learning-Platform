<template>
  ...
  <!-- TODO: check if audio in dictionary is working -->
  <div>
    ...
    <div id="popup-modal" tabindex="-1" class="fixed inset-0 flex items-center justify-center z-50 my-auto mx-2"
      v-if="showModal">
      <div class="bg-bgColor max-w-3/4 md:max-w-3xl">
        ...
        <div v-if="$page.props.auth.user">
          <form>
            <label class="relative inline-flex items-center cursor-pointer mt-4 ml-4">
              ...
              <div :class="{ 'bg-gray-400': !added, 'bg-mainColor': added }"
                class="w-11 h-6  peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-mainColor">
              </div>
              <span class="ml-3 text-sm font-medium text-darkestColor" v-if="!loading">
                {{ added === false ? 'Add word to dictionary' : 'Word added to dictionary' }}
              </span>
              <span class="ml-3 text-sm font-medium text-darkestColor" v-else>
                Wait a second...
              </span>
            </label>
          </form>
          ...
          <li v-if="loading" class="my-4">
            <div role="status" class="flex items-center">
              <svg aria-hidden="true"
                class="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-mainColor"
              ...
              <span class="ml-2 text-sm font-medium text-darkestColor">Loading...</span>
            </div>
          </li>
          <div v-else>
            <p v-if="!data">
            ...
            <li v-else v-for="entry in data" :key="entry.word" class="my-4">
              ...
              <ul>
                <li v-for="meaning in entry.meanings" :key="meaning.partOfSpeech"
                  class="mt-4 py-2 text-sm md:text-base border-b md:w-max-2/3 border-dashed border-darkColor">
                  <small>{{ meaning.partOfSpeech }}</small>
                  <ul>
                    <li v-for="definition in meaning.definitions" :key="definition.definition" class="m-2">
                      <div class="text-justify indent-2 md:indent-4">{{ definition.definition }}</div>
                    </li>
                  </ul>
                </li>
              </ul>
              <div class="bg-lightColor p-2 mt-2">
                <p class="md:m-3 font-semibold underline">Pronunciation:</p>
                <div v-for="phonetic in entry.phonetics" :key="phonetic.text"
                  class="md:m-4 flex flex-col md:flex-row justify-between items-center">
                  <p class="md:mr-4">{{ phonetic.text }}</p>
